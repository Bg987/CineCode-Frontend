...
const socket = io("http://192.168.254.47:4000");
    ...
    useEffect(() => {
        ...
        socket.on("dashboard", (data) => {
            ...
            if (data.LongestMovie?.Mid) {
                setLurl(`http://192.168.254.47:4000/apiSeeM/images/${data.LongestMovie.Mid}.jpg`);
            }
            if (data.SmallestMovie?.Mid) {
                setSurl(`http://192.168.254.47:4000/apiSeeM/images/${data.SmallestMovie.Mid}.jpg`);
            }
        });
    ...
    return (
        <Box sx={{minHeight: "100vh", py: 3 }}>
            ...
            <Grid container spacing={2} justifyContent="center" sx={{ mt: 3 }}>
                {["Add Movie", "Approve Movies", "Delete Movie"].map((label, index) => (
                    <Grid item key={index}>
                        <Button
                            variant="contained"
                            component={Link}
            ...
            <Grid container spacing={2} sx={{ mt: 4, px: 3 }}>
                {[
                    ...
                    { label: "Total Movies", value: dashboardData.movieNo?.count || "--" },
                    { label: "Admin Movies", value: dashboardData.AdminMovieNo?.count || "--" },
                    { label: "Approved Movies", value: dashboardData.AppMovieNo?.count || "--" },
